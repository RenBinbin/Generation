...
/**
 * Created by renbinbin1 on 2017/3/30.
 */
...
public class MySlideView extends View {
    ...
    private void initView() {
        ...
        mBound = new Rect();
        backgroundColor = getResources().getColor(R.color.bg3);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < CityActivity.pinyinList.size(); i++) {
            ...
            if (i == position - 1) {
                ...
                paint.setColor(getResources().getColor(R.color.textcolor5));
            }
            paint.setTextSize(29);
            paint.getTextBounds(textView, 0, textView.length(), mBound);
            canvas.drawText(textView, (mWidth - mBound.width()) * 1 / 2, mTextHeight - mBound.height()+10, paint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                backgroundColor = getResources().getColor(R.color.bg4);
                mTextHeight = mHeight / CityActivity.pinyinList.size();
